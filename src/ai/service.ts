...
export class AIService {
  ...
  async generateCollection(description: string, options?: { dryRun?: boolean }): Promise<CollectionData> {
    const systemPrompt = `You are a database schema designer. Generate a JSON collection schema for TspoonBase (a PocketBase-like backend).
...
Rules use PocketBase filter syntax: @request.auth.id, @request.auth.isAdmin, =, !=, >, <, ~ (contains), &&, ||, !().
...
Do not include markdown, explanations, or comments. Only raw JSON.`
  ...
  async generateRule(action: string, description: string): Promise<string> {
    const systemPrompt = `You translate plain English security requirements into PocketBase filter expressions.
...
You can help with:
- Writing PocketBase filter expressions
